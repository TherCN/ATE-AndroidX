...
public:
    ...
    void set(const dert* o, size_t numChars) {
        if (mString) {
            free(mString);
        }
...
JNIEXPORT void JNICALL Java_thercn_terminal_TermExec_sendSignal(JNIEnv *env, jobject clazz,
    jint procId, jint signal)
{
    kill(procId, signal);
}

JNIEXPORT jint JNICALL Java_thercn_terminal_TermExec_waitFor(JNIEnv *env, jclass clazz, jint procId) {
    int status;
...
JNIEXPORT jint JNICALL Java_thercn_terminal_TermExec_createSubprocessInternal(JNIEnv *env, jclass clazz,
